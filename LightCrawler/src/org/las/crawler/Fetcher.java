...
public final class Fetcher {
	...
	public Fetcher(){
		...
		params.setIntParameter("http.socket.timeout", Config
				.getIntProperty("fetcher.socket_timeout", 20000));

		params.setIntParameter("http.connection.timeout", Config
				.getIntProperty("fetcher.connection_timeout", 30000));

		params.setBooleanParameter("http.protocol.handle-redirects",
				Config.getBooleanProperty("fetcher.follow_redirects",
						true));
		params.setBooleanParameter("http.protocol.allow-circular-redirects",
				Config.getBooleanProperty("fetcher.allow-circular_redirects",
						true));

		ConnPerRouteBean connPerRouteBean = new ConnPerRouteBean();
		connPerRouteBean.setDefaultMaxPerRoute(Config.getIntProperty(
				"fetcher.max_connections_per_host", 100));
		ConnManagerParams.setMaxConnectionsPerRoute(params, connPerRouteBean);
		ConnManagerParams.setMaxTotalConnections(params, Config
