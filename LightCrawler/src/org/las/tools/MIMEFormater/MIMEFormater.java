package org.las.tools.MIMEFormater;
...
public class MIMEFormater {
	...
	public MIMEFormater() {
	    ...
	    try {
	      p.load(this.getClass().getResourceAsStream("mimemappings.properties"));
	    }catch (Exception e) {
			// TODO: handle exception
		}
	    Enumeration<?> alltypes = p.keys();
	    while(alltypes.hasMoreElements()){
	    	String mime = (String) alltypes.nextElement();
	    	String format = (String) p.get(mime);
	    	mimeMap.put(mime, format);
	    }
	}
	...
	public String JudgeFormat(String url, String contentType) {
		String type = null;
		if(contentType!=null){
			type = contentType.toLowerCase().trim();
			type = mimeMap.get(contentType);
			if(type == null || type.length()==0){
				type = "html";
			}
		}else{
			type = JudgeURLFormat(url);
			if(type == null || type.length()==0){
				type = "html";
			}
		}
	...
	public String JudgeHttpFormat(String url, String contentType) {
		String type = null;
		if(contentType!=null){
